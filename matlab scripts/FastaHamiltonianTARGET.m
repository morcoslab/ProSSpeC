function H=FastaHamiltonianTARGET(inputfile,couplings,localfields,Htype,N1,stype, maskedPositions)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for seq=1:M
        for res=1:N
            if (~ismember(res, maskedPositions))
    ...
    switch Htype
            ...
            %Compute across 2 different species
              for seq=1:M
                  for res=1:N1
                      if (~ismember(res, maskedPositions))
                          for pair=(N1+1):N
                              if (~ismember(pair, maskedPositions))
        ...
        case 2
            ...
            for seq=1:M
                  for res=1:N
                      if (res<N) && (~ismember(res, maskedPositions))
                          for pair=(res+1):N
                              iindex=q*(res-1)+Sequences(seq,res);
                              jindex=q*(pair-1)+Sequences(seq,pair);
                              H(seq)=couplings(iindex,jindex)+H(seq);
